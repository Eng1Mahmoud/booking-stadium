 ...
 * fixed length — an hour of maintenance and a whole closed day are both normal —
 * so duration chips can't express it. It is a range: first hour, then last.
 *
 * The second press names the last hour to *close*, not the moment to stop, so
 * "one to three" lights up exactly the three buttons it closes. Deriving the end
 * from that is also how blocking until midnight reaches 24:00 unaided.
 */

const props = defineProps<{
  /** Context only — staff may close an hour that already holds a booking. */
...
const cells = computed<HourCell[]>(() =>
  Array.from({ length: 24 }, (_, hour) => {
    // A half-taken hour reads as taken: that is exactly the case staff need to
    // notice before closing it.
    const onHour = statusAt(hour * 60)
    const halfHour = statusAt(hour * 60 + 30)
    const status =
      (onHour === 'available' || onHour === undefined ? halfHour : onHour) ?? 'available'

    const hours12 = hour % 12 === 0 ? 12 : hour % 12
    return {
      hour,
      label: `${hours12} ${hour < 12 ? 'ص' : 'م'}`,
      // Elapsed hours have nothing left to close. Hours holding a booking stay
      // open: closing one is a deliberate act the dashboard exists for.
...
/** 23 closes at 24:00 — the day's end, which "12 ص" would read as its start. */
const endLabel = computed(() => {
  if (props.endHour === null) return ''
  return props.endHour === 23
    ? 'منتصف الليل'
    : formatTime12h(toTimeString((props.endHour + 1) * 60))
})
...
function pick(cell: HourCell) {
  ...
  // Pressing earlier than the start reads as changing your mind about where to
  // begin, not as an invalid range.
...
<template>
  <div class="space-y-3">
    <!-- The instruction tracks the step, so the two-press sequence never
    ...
    >
      ...
      <template v-else-if="!isComplete">
        ...
        <span class="block text-chalk-400">
          اضغط على <strong class="text-chalk-200">آخر ساعة</strong> تريد إغلاقها. لإغلاق ساعة واحدة
          فقط، اضغط على <strong class="text-chalk-200">{{ startLabel }}</strong> مرة أخرى.
        </span>
      </template>
      ...
      <template v-else>
        ...
        <span class="mt-1 block text-xs text-chalk-500"> اضغط أي ساعة لبدء تحديد جديد. </span>
      </template>
    </div>
